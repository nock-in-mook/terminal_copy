...
**launcher 起動経路（超重要）:**
launcher は **Hammerspoon が AppleScript 経由で iTerm に依頼して spawn させる**。Hammerspoon は LSUIElement なので直接 spawn すると `screencapture -i` の TCC judge が通らず、`open -a SokuLauncher.app` / launchd 経由は LaunchServices 由来 chain で長時間後に TCC が陳腐化する（いずれも #15）。前景アプリ iTerm 経由なら TCC chain が健全。SokuLauncher.app のログイン項目登録は 2026-04-19 に廃止済み。

iTerm 経由 spawn の具体: `hs.osascript.applescript` で `tell application "iTerm" ... create window ... write text "(nohup arch -arm64 python3 folder_launcher.py ... &); exit"` を送る。iTerm session 内のコマンドは `& disown` で分離された後 `exit` で session を畳むので、iTerm window は即 close され、launcher 本体は `_SOKU_DAEMONIZED` フラグによる `subprocess.Popen` 自己 re-exec で iTerm から完全に独立する。ユーザー視点では iTerm ウィンドウは±0（増減なし）。
...
- **対処（2026-04-19 に根本解決）**:
  - **SokuLauncher.app のログイン項目登録を廃止**。代わりに **Hammerspoon が AppleScript で iTerm に依頼して launcher を spawn** する方式に変更
  - Hammerspoon 直接 spawn は試したが失敗（Hammerspoon は LSUIElement＝Dock 非表示のバックグラウンドアプリなので、そこから生まれた子の `screencapture -i` は overlay UI が却下される模様）。iTerm は前景アプリなので chain が健全
  - AppleScript で iTerm window を一時的に作り、shell に nohup & disown で launcher を起こして即 exit。iTerm window は自動で閉じ、launcher は `_SOKU_DAEMONIZED` フラグで `subprocess.Popen` 自己 re-exec して iTerm から独立する
  - 自動復旧: 透明キーボード側で `could not create image from rect` を検知したら `/tmp/sokulauncher_restart_requested` を touch（`_trigger_recovery()` in `transparent_keyboard_mac.py`）。Hammerspoon が 3 秒ポーリングで検知して launcher+透明キーボードを `pkill -9` → AppleScript 経由で再 spawn
  - 予防: Hammerspoon の `hs.timer.doAt("04:00", "1d", ...)` で毎日4時に予防的再起動
  - daemonize 方法の注意: **`os.fork()` は使えない**。AppKit import 済みの Python で fork すると Apple Silicon 上で `NSResponder initialize ... fork() called ... Crashing` で即 crash する（Objective-C runtime と pthread の競合）。必ず `subprocess.Popen` で exec し直す方式を使う
