...
## 今回の修正（セッション021）
- デスクトップアイコン上のダブルクリックでメニューが出るバグ修正
- 原因: CGWindowListのlayer 0チェックだけではFinderが管理するデスクトップアイコンを検出できない
- 修正: `_is_desktop_click`にFinderの選択状態チェックを追加（アイコンクリック時はselection > 0なのでメニュー表示をスキップ）
